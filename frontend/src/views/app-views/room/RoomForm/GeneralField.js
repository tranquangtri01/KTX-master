    ...
    <Row gutter={16}>
      <Col xs={24} sm={24} md={17}>
        <Card title="Thông tin chi tiết">
          <Form.Item name="name" label="Tên phòng" rules={rules.name}>
            <Input placeholder="Tên phòng" />
          </Form.Item>
          <Form.Item name="campus" label="Cơ sở quản lý" rules={rules.campus}>
          ...
          <Form.Item name="roomType" label="Loại phòng" rules={rules.roomType}>
            <Select className="w-100">
              {roomType.length &&
                roomType.map((item) => (
                  <Option key={item.id} value={item.id}>
