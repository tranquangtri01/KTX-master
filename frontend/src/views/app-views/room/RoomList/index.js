...
const ProductList = () => {
  ...
  const fetchAllRoom = async () => {
    try {
      const res = await axios.get(URL + "room");
      setList(res.data);
  ...
  const dropdownMenu = (row) => {
    return (
      <Menu>
        <Menu.Item>
          <Link to={"/app/room/detail/" + row.id}>
        ...
        <Menu.Item>
          <Link to={"/app/room/create-bill/" + row.id}>
            <Flex alignItems="center">
              <DiffOutlined />
              <span className="ml-2">Tạo hóa đơn tháng này</span>
            </Flex>
          </Link>
        </Menu.Item>
        <Menu.Item>
          <Link to={"/app/room/edit/" + row.id}>
            <Flex alignItems="center">
              <EditOutlined />
  ...
  const tableColumns = [
    ...
    },
    {
      ...
      sorter: (a, b) => utils.antdTableSorter(a, b, "campusName"),
    },
    {
      title: "Loại phòng",
      dataIndex: "roomName",
